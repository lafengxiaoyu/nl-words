...
**采样率配置**：
- 默认：1%（可通过 `VITE_API_LOG_SAMPLING_RATE` 环境变量调整）
- 设置为 `1.0` 记录所有操作（不推荐）
- 设置为 `0.1` 记录 10% 操作
- 设置为 `0.01` 记录 1% 操作（推荐，默认值）
- 设置为 `0.001` 记录 0.1% 操作（超轻量）
...
SELECT
  ...
  COUNT(*) * 100 as estimated_total  -- 乘以 100 估算总数（1% 采样率）
...
**注意**：由于采用采样模式，显示的调用次数需要乘以采样率的倒数来估算实际调用次数。例如 1% 采样率显示 100 次，实际约为 10,000 次。
...
2. **存储成本**：建议定期清理旧日志（7-30 天），使用 1% 采样率可大幅降低存储需求
3. **采样率**：默认 1% 采样，适合趋势分析但不适合精确计费
4. **未登录用户**：未登录用户使用 localStorage，不会产生 API 调用
5. **错误处理**：日志记录失败不应影响主功能
