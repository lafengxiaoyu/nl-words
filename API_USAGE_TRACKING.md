...
**采样率配置**：
- 默认：5%（可通过 `VITE_API_LOG_SAMPLING_RATE` 环境变量调整）
- 设置为 `1.0` 记录所有操作（不推荐）
- 设置为 `0.1` 记录 10% 操作
- 设置为 `0.05` 记录 5% 操作（推荐，默认值，降低 95%）
...
SELECT
  ...
  COUNT(*) * 20 as estimated_total  -- 乘以 20 估算总数（5% 采样率）
...
**注意**：由于采用采样模式，显示的调用次数需要乘以采样率的倒数来估算实际调用次数。例如 5% 采样率显示 100 次，实际约为 2,000 次。
...
2. **存储成本**：建议定期清理旧日志（7-30 天），使用 5% 采样率可降低 95% 存储需求
